...
echo "[10] Vehicle 1; model=${MODEL}; runtime=${ZHIHANG_YOLO_RUNTIME}/${RUNTIME_ENV}; endpoint=${HOST}:${PORT}"
cd "${ROOT}"
bash preflight_vehicle.sh 1 "${PORT}" "${HOST}"
sleep 3
exec bash run_vehicle_terminal.sh 1 "${RUNTIME_ENV}" "${MODEL}" "${PORT}" "${HOST}"

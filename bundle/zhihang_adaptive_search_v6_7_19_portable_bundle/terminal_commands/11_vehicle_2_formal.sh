...
echo "[11] Formal Vehicle 2; model=${MODEL}; runtime=${ZHIHANG_YOLO_RUNTIME}/${RUNTIME_ENV}; endpoint=${HOST}:${PORT}"
cd "${ROOT}"
bash preflight_vehicle.sh 2 "${PORT}" "${HOST}"
sleep 2
exec bash run_vehicle_terminal_formal.sh 2 "${RUNTIME_ENV}" "${MODEL}" "${PORT}" "${HOST}"
